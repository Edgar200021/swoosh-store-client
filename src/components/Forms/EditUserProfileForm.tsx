      ...
      >
        <fieldset disabled={isLoading}>
          ...
          <div className='mb-8'>
            {avatar ?
                <div
                    className='border-[1px] border-dashed border-black bg-slate-200 p-4 flex items-center juisty-center gap-x-5'>
                  <img className='w-16 h-16 rounded-full object-cover' src={URL.createObjectURL(avatar)}
                       alt={avatar.name}/>
                  <span>{avatar.name}</span>
                  <Button onClick={() => setAvatar(undefined)}
                          className='w-6 h-6 ml-auto hover:translate-y-[-6px] active:translate-y-[-3px] transition-transform duration-300 ease'
                          variant='clear'>
