...
export const SneakersPage = () => {
  // const filterObj1 = useGetSearchParams(keys<ISneakerFilter>().join(','))

  const filterObj = useGetSearchParams('size', 'colors', 'limit', 'page', 'price>=', 'price<=', 'color', 'material', 'for', 'sort')

  return (
      <main className='pt-48 md-tablet:pt-24'>
        <Collapsible    renderTrigger={(fn, isCollapsed) =>
            <Button className='hidden desktop:flex justify-center items-center gap-x-3 py-5 border-[1px] border-solid border-[#EAEAEA]  w-full ' onClick={() => fn(prev => !prev)} variant='clear'>
          ...
          <SneakerFilter className='mb-20 overflow-hidden'/>
        </Collapsible>
        <SneakerFilter className='desktop:hidden'/>
